...
export const editExerciseRoute = async (context: Context): Promise<Response> => {
	const { verifyToken } = await import('../../utils/auth');
	try {
		const { req: request, env } = context;
		const { exercise_id, exercise, workout_id } = await request.json();
		...
		const result = await stub.editExercise(reps, sets, weight, name, exercise_id);
